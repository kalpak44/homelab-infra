...
**`mac-calendar-mcp` cuts a version per merge, and the version commit must not re-enter the pipeline.** `release.yml` is its CI, its `PR_CHECK_WORKFLOW` and its release in one: verify (`npm test` at an 80% threshold in `jest.config.js`, then the bundle build) → version → release. The number is computed in bash by diffing the direct dependency majors in `package.json` between `HEAD~1` and `HEAD`, mapped through SemVer §4 — a major moving bumps the minor at `0.y.z`, everything else the patch — never by the agent. The agent merges major bumps here, which is why each one must arrive as its own PR rather than inside the group. Two things were measured the hard way on 2026-09-16: the version commit is itself a push to main that changes `package.json`, so without `[skip ci]` it re-fires the gate and bumps for ever (it reached 0.1.3 in under two minutes), and `git push --follow-tags` silently ignores a lightweight tag, so `git tag` must be `git tag -a` or the release job cannot check the tag out. `README.md` is deliberately *not* in the gate's ignore list: `build-release.mjs` copies it into the zip, so a README fix is a shipped change. Before this existed the repo had no `pull_request` workflow at all, so the agent never merged anything and every push re-uploaded the asset of one permanent `v0.1.0` — created in May, its zip last overwritten in September.

**`code-viewer-bot` needs a second workflow, because its publisher cannot be a PR check.** `publish-marketplace.yml` is `workflow_dispatch` only and takes a human-chosen version, so there is nothing the agent could start on a branch; `verify.yml` is the `PR_CHECK_WORKFLOW` instead — `format:check`, `lint`, jest and a waited-on SonarCloud gate. Three things about this repo are not obvious. Its `npm ci` must pass `--ignore-scripts`: `robotjs` runs `node-gyp rebuild` at install time, which needs X11 headers and does not build on Node 24 — `SKIP_ROBOTJS_REBUILD=1` only skips the *postinstall*, not that. `.prettierignore` must list `.github`, or prettier reformats the generated workflows, the next apply restores Terraform's copy, and `format:check` then fails on files no one can fix in the repo — blocking every merge. And `.gitignore` used to contain `test/`, which silently held the whole suite out of git: the tests passed locally and CI reported `No tests found, exiting with code 1`. Coverage thresholds are per-path with no `global` block on purpose — Jest removes path-matched files from the global pool, so a global floor would measure only `extension.js`, `mouse-bot.js`, `config-panel.js`, `workspace-navigator.js` and `instance-coordinator.js`, the modules that require `vscode` or `robotjs` and cannot load outside a VS Code host.
