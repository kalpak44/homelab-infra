...
**The syft and grype pins move on a weekly `just bump-scanners` instead, because nothing else can reach them.** They sit in seven generated workflow files, and each publishing repo pins the *installer script's* SHA256 alongside the version — so a plain version bump leaves a checksum that no longer matches and every scan step fails at once. The recipe resolves each tool's latest release, recomputes that checksum from the tag it is about to pin, installs and runs the binary before keeping it, rewrites only the files that actually move, commits, and then applies those repos — a commit here changes nothing in the target repo until Terraform pushes the file into it. Its rollback restores saved copies rather than running `git checkout --` on the directory, which would discard unrelated uncommitted work sitting beside it. Drift is what this replaces: the two container-image repos sat 23 syft minors behind while the five publishing ones were current.

**`paths-ignore` on `push` is what stops a Terraform sync from rebuilding and redeploying.** Syncing the agent prompt, the agent workflow or `dependabot.yml` changes nothing that ends up in an image, and without the filter each of those pushes ran a full build, publish and cluster rollout. It must be on `push` alone: a pull request whose check never runs is one the agent will never merge, since it requires a green one. And it must skip the run outright rather than gate the publish step — `gitops/Justfile`'s `bump-images` takes its tag from the last successful *push* run, so a successful run that published nothing points the cluster at a tag that does not exist. `publish.yml` itself is deliberately absent from every such list, so a scanner-pin bump still rebuilds and rescans.
...
**`kubectl-awscli` no longer ships `aws-cli`; `postgres-awscli` still does, and the difference is measured rather than argued.** On Alpine the CLI is the Python build, and `py3-jmespath` plus `py3-cryptography` carried both Criticals and four of the five Highs, none of them with a fix Alpine had packaged. Dropping it from `kubectl-awscli` took the image from 65 MB to 25 MB and from 2 Critical / 5 High to a single unfixable `zlib` High.

**Do not swap it for a Go S3 client in `postgres-awscli` — that was tried, measured, and reverted.** `minio-client` is a third of the size and pulls 58 fewer packages, which is what made it look obvious. But grype catalogs a Go binary's vendored modules, and `golang.org/x/crypto` with `grpc` scan as **8 Critical with no reachable fix** against `aws-cli`'s 2. Measured on one grype database, linux/amd64: aws-cli 2C/6H at 50 MB, minio-client 8C/17H at 21 MB, rclone 8C/24H at 46 MB, no client at all 0C/1H at 12 MB. The publish gate refused the candidate, correctly. Package count and image size are not a proxy for scan surface, and a client that ships its dependencies inside the binary hides them from every lever this pipeline has.

**The only measured route to 0C/1H keeps the image and drops the client**, which means signing the three S3 calls in the scripts. Not attempted here; it trades an unfixable CVE surface for hand-written SigV4, and that trade needs a decision rather than a commit.
