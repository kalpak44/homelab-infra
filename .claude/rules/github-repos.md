...
**There are two `ai-pr-agent.yml` variants, and they differ only in their tail.** Both are the same 900-odd-line prompt: sweep bot PRs oldest first, repair the branch, read the Sonar gate from the API, wait on the final head commit, merge. The *releasing* variant (`mac-calendar-mcp`, `code-viewer-bot`) ends by writing a bump word and cutting one tag per sweep. The *publishing* variant (`kalpak44`, `bunker-party`, `mite-assistant-mcp`, `proklinator-app`, `noco-google-connector-web-page`) has no release step at all — those repos ship by publishing an image from main — and instead ends by acting on the image scan. Each variant is byte-identical across its repos apart from the banner, so a fix is a copy.
