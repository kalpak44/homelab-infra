...
**Its quality gate is `format:check`, `lint` and a waited-on SonarCloud gate — all blocking, none of which existed in the repo.** ESLint and Prettier were added to `mite-assistant-mcp` itself on 2026-09-09 (not Terraform-managed: Terraform owns only `.github/`, and Dependabot has to be able to bump them). The agent prompt names those exact commands and sanctions `npm run format` / `npm run lint:fix` on the PR branch, because the agent merges with no human review and its own fixes are the only code nothing else checks. `SONAR_TOKEN` goes in both secret stores for the usual reason, and the analysis step is guarded on `SONAR_PROJECT_KEY != ''` as well — a missing project key downgrades the gate to nothing while the check still goes green, so verify the step ran, not that the run passed.
