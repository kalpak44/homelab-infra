...
| `kalpak44/code-viewer-bot`                | `github/code-viewer-bot`                | same, plus **every workflow the repo has**: `verify.yml` (its PR check — `format:check`, `lint`, jest with per-path coverage floors and a waited-on SonarCloud gate) and `publish-marketplace.yml` (manual, versioned, builds four platform VSIX files and publishes to the VS Code Marketplace), plus `dependabot.yml`. Also gets `SONAR_TOKEN` in both secret stores |
...
| `kalpak44/mac-calendar-mcp`               | `github/mac-calendar-mcp`               | same, plus `release.yml` (its CI, its PR check and its release in one — `npm test` at an 80% coverage threshold and the bundle build, then a version computed in bash and a tagged GitHub release carrying the zip) and `dependabot.yml`. A dependency major moving bumps the minor, everything else the patch; the agent merges majors too. Also gets `GH_ADMIN_TOKEN`, which is what lets the version commit reach main |
