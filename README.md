...
| `kalpak44/mite-assistant-mcp` | `github/mite-assistant-mcp` | same, plus `publish.yml` (its CI, its PR check, and its deploy trigger in one — `format:check`, `lint` and a waited-on SonarCloud quality gate, then publishes `ghcr.io/kalpak44/mite-assistant-mcp` and dispatches **GitOps - Bump images** for `mite-assistant-mcp`) and `dependabot.yml`, which proposes at 05:00 Europe/Sofia for the agent's 04:00 UTC sweep. Every merge in that sweep publishes its image but holds the rollout; the agent dispatches one rollout of the batch when it finishes. Also gets `GH_ADMIN_TOKEN` and `SONAR_TOKEN` (both secret stores) |
