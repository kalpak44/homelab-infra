...
| `kalpak44/plugin-noco-tools`  | `github/plugin-noco-tools`  | same, plus `site.yml` (its PR check, which publishes `ghcr.io/kalpak44/noco-tools-site` and dispatches **GitOps - Bump images** for `noco-google-connector-site`) and `dependabot.yml`. Also gets `GH_ADMIN_TOKEN` for that dispatch |
