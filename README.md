...
| `kalpak44/noco-google-connector-web-page` | `github/noco-google-connector-web-page` | same, plus `publish.yml` (its PR check, which publishes `ghcr.io/kalpak44/noco-tools-site` and dispatches **GitOps - Bump images** for `noco-google-connector-web-page`) and `dependabot.yml`. Also gets `GH_ADMIN_TOKEN` for that dispatch |
