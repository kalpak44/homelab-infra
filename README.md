...
| `kalpak44/mite-assistant-mcp` | `github/mite-assistant-mcp` | same, plus `publish.yml` (its CI, its PR check, and its deploy trigger in one — publishes `ghcr.io/kalpak44/mite-assistant-mcp` and dispatches **GitOps - Bump images** for `mite-assistant-mcp`) and `dependabot.yml`. Also gets `GH_ADMIN_TOKEN` for that dispatch |
